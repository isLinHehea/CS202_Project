...
loadword1:
	...
	beq $t4,$26,back1 # whether I finish reading0
	#srl $a0,$t2,8
	#sw $a0,0xC50($1)
	j loadword1
...
print0:
 ...
 sw $t5, 0XC92($1) # print sum
 sw $t5, 0XC52($1) # print sum
 j forever
...
on: 
 ...
 beq $t6,$t7,off
 sw $t2, 0XC60($1)
 sw $t4, 0XC92($1) # print sum
 sw $t4, 0XC52($1) # print sum
 addi $t6,$t6,1
 addi $t6,$t6,-1
 addi $t6,$t6,1
 j on
...
off:
 ...
 beq $t8,$t7,forever
 sw $0, 0XC60($1) ###
 sw $0, 0XC92($1) # print sum
 sw $0, 0XC52($1) # print sum
 addi $t8,$t8,1
 # addi $t6,$t6,1
 # addi $t6,$t6,-1
 j off
...
case1:
	...
	sw $t5, 0XC60($1) 
	sw $t5, 0XC90($1) # print sum
	sw $t5, 0XC50($1) # print sum
	j forever
...
loop2:
	...
	sw $a0, 0XC90($1) # print sum
	sw $a0, 0XC50($1) # print sum
	addi $t6,$t6,1
	j loop2
...
quo:
	...
	sll $t3,$s0,24
	sra $t3,$t3,24
	sw $t3, 0XC92($1) # print sum
	sw $t3, 0XC52($1) # print sum
	addi $t8,$t8,1
	j quo
